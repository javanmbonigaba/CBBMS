...
def donate_blood_view(request):
    ...
    if request.method == 'POST':
        ...
        if donation_form.is_valid():
            ...
            try:
                # Fetch the Donor object associated with the current user
                donor = models.Donor.objects.get(user_id=request.user.id)
                blood_donate.donor = donor
                blood_donate.save()
                return HttpResponseRedirect('donation-history')
            except models.Donor.DoesNotExist:
                # If no Donor object is found, redirect to an error page or show a message
                return render(request, 'donor/not_found.html', {'error': 'Donor profile not found. Please ensure you have completed your donor registration.'})
